...
class PostController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (business_config('suspend_on_exceed_cash_limit_provider', 'provider_config')->live_values) {
            $providers = Provider::with('owner')->whereIn('id', $provider_ids)->coveringLeafZone($post->zone_id)->where('app_availability', 1)->where('is_suspended', 0)->get();
        } else {
            $providers = Provider::with('owner')->whereIn('id', $provider_ids)->coveringLeafZone($post->zone_id)->where('app_availability', 1)->get();
        }
