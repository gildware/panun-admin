            ...
            <?php if (($tab ?? '') === 'chats' || ($tab ?? '') === 'human_support'): ?>
                <div class="row g-2 align-items-stretch wa-chats-split-layout">
                    <?php
                        ...
                        $waFilterAllLabel = str_replace(':count', (string) (int) ($chatCounts['total'] ?? 0), translate('whatsapp_active_chats_filter_all'));
                        $waFilterUnreadLabel = str_replace(':count', (string) (int) ($chatCounts['unread'] ?? 0), translate('whatsapp_active_chats_filter_unread'));
                    ?>
...
(function() {
    ...
    function waRenderAdAttribution(attr) {
        ...
        var platformLabel = (attr.platform_label && String(attr.platform_label).trim()) || '{{ translate('WhatsApp Ad') }}';
        var displayName = (attr.display_name && String(attr.display_name).trim())
