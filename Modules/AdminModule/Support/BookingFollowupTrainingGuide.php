...
class BookingFollowupTrainingGuide
{
    ...
    private static function slideMetaMap(): array
    {
        return [
            ...
            'follow-up' => ['icon' => 'schedule', 'overview' => 'Three touchpoints plus Add vs Take follow-up modals — schedule, log Taken/Reschedule, history table, and alerts.'],
    ...
    private static function slideDeckGuide(): array
    {
        return [
            ...
            'terms' => [
                ...
                ['term' => 'Follow-ups tab', 'definition' => 'Booking subpage + Activity pill — history table, Add Follow-up (schedule), Take Follow-up (log Taken or Reschedule).'],
            ...
            'ui_maps' => [
                ...
                self::panelMap(
                    ...
                    [
                        ...
                        ['label' => 'Follow-ups tab → Add Follow-up', 'text' => 'Log first touchpoint — For, Taken + Call/WhatsApp (+ recording if call), Remarks, Next date. Or Take on scheduled row later.'],
    ...
    private static function slideFollowUp(): array
    {
        return [
            ...
            'shift_checklist' => [
                ...
                'Missed / Pending banners on booking → Take Follow-up before new Add',
                'Service day: provider AM → customer if needed → WhatsApp 1 hr before → Ongoing',
                'After job: provider bill → panel → customer confirm',
            ],
            'panel_links' => [
                self::panelLink('Booking list → Follow-ups tab', '/admin/booking/list?booking_status=accepted&service_type=all'),
                self::panelLink('Today\'s booking follow-ups', '/admin/booking/todays-followups'),
            ],
            'card_groups' => [
                [
            ...
            'ui_maps' => [
                ...
                self::panelMap(
                    ...
                    'Follow-ups subpage → **Add Follow-up**. Lead-style modal — log **Taken** with Follow up Taken on (Call/WhatsApp), optional **Voice Recording** on Call, Remarks, and Next Follow-up Date.',
                    [
                        ['label' => 'For *', 'text' => 'Customer or Provider — who you contacted.'],
                        ['label' => 'Status — Taken', 'text' => 'Contact happened. Default when logging after call or WhatsApp.'],
                        ['label' => 'Status — Reschedule', 'text' => 'No contact yet — pick future date only (hides Date Time + Taken on).'],
                        ['label' => 'This Follow-up — Date Time *', 'text' => 'When contact happened (Taken). Defaults to now.'],
                        ['label' => 'Follow up Taken on *', 'text' => '**Call** or **WhatsApp**. Call shows **Voice Recording** upload (optional, max 10 MB).'],
                        ['label' => 'Voice Recording', 'text' => 'Optional on Call — upload then Transcribe from history row View button.'],
                        ['label' => 'Remarks', 'text' => 'What was said and outcome. Example: “Provider confirmed Thu 10 AM Rajbagh.”'],
                        ['label' => 'Next Follow-up Date *', 'text' => 'Required on open bookings — schedules next touch for that party.'],
                        ['label' => 'Save changes', 'text' => 'Taken → Completed row with Call/WhatsApp in history. Next date → new Scheduled row.'],
                    ],
                    'booking-followup-add-modal.png',
                ),
                self::panelMap(
                    'Follow-ups tab — Take Follow-up modal (log contact)',
                    'Click Take Follow-up on a scheduled row, alert banner, or provider/customer card. Same lead-style modal — log what happened and set next date.',
                    [
                        ['label' => 'Context line', 'text' => 'Top of modal — For (Customer/Provider) · Scheduled for · Reason from Add step.'],
                        ['label' => 'Status — Taken', 'text' => 'Contact happened (call or WhatsApp). Default when opening Take modal.'],
                        ['label' => 'Status — Reschedule', 'text' => 'Could not reach — no contact yet. Hides Date Time + channel; **Reschedule to** date required instead.'],
                        ['label' => 'This Follow-up — Date Time *', 'text' => 'When contact happened (Taken only). Defaults to now.'],
                        ['label' => 'Follow up Taken on *', 'text' => 'Call or WhatsApp (Taken only). Call shows optional Voice Recording upload (max 10 MB).'],
                        ['label' => 'Remarks', 'text' => 'What was said, outcome, next action. Example: “Called provider — confirmed Thu 10 AM.” Handover for next shift.'],
                        ['label' => 'Next Follow-up — Urgency', 'text' => 'Priority for the upcoming touch. High for same-day service.'],
                        ['label' => 'Next Follow-up Date *', 'text' => 'Required on **open bookings** until Completed/Canceled. Example: service day 8:30 AM.'],
                        ['label' => 'Schedule another follow up', 'text' => 'Checkbox on closed bookings only — when mandatory next is off. Open bookings always require next date.'],
                        ['label' => 'Save changes', 'text' => 'Taken → row Completed + next scheduled if date set. Reschedule → row rescheduled + new scheduled date.'],
                    ],
                    'booking-followup-take-modal.png',
                ),
                self::panelMap(
                    'Follow-up history table — columns',
                    'Follow-ups subpage or Activity → Followups pill. Work Scheduled rows first via Take Follow-up.',
                    [
                        ['label' => 'Scheduled for', 'text' => 'Planned contact date from Add Follow-up.'],
                        ['label' => 'Taken on', 'text' => 'Actual contact time after Take → Taken.'],
                        ['label' => 'Delay', 'text' => 'On time or Delayed by X — compares taken vs scheduled.'],
                        ['label' => 'Next Follow-up Date', 'text' => 'Next touch you set in Take modal.'],
                        ['label' => 'For / Urgency / Taken By / Status', 'text' => 'Party, priority chip, staff name, Scheduled / Completed / Rescheduled.'],
                ...
                self::panelMap(
                    ...
                    'Banner on booking details or Follow-ups subpage when a scheduled follow-up is due or overdue — per Customer and Provider party.',
                    [
                        ['label' => 'Pending Follow-up', 'text' => 'Due today or later — yellow banner with Take Follow-up button.'],
            ...
            'remember' => [
                'Add Follow-up = log Taken with Follow up Taken on (Call shows recording field)',
                'Take Follow-up = same modal on a scheduled row — context line at top',
                'Open bookings: Next Follow-up Date required on every save until closed',
                'Day-before only when service is 3+ days out',
                'Provider first on service day; log remarks every contact',
            ],
            'avoid' => [
                'Logging only in remarks — every contact needs a Take Follow-up row',
                'Take → Taken without next date on open booking',
                'Daily calls with no new info',
                'Day-before call for tomorrow booking',
            ],
        ];
    }
