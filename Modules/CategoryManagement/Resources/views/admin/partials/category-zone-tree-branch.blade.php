...
@foreach($nodes as $node)
    @if(! empty($node['children']))
        <div class="category-zone-tree-item border-bottom border-light" style="padding-left: {{ $level * 0.85 }}rem;">
            <div class="d-flex align-items-center gap-2">
                <button
                    type="button"
                    class="category-zone-tree-toggle btn btn-light border-0 flex-shrink-0 rounded-0 py-2 px-0 d-flex align-items-center justify-content-center"
                    style="width: 34px; background: #f8f9fa;"
                    aria-expanded="false">
                    <span class="material-icons category-zone-chevron text-muted"
                ...
                <div class="form-check py-2 mb-0 flex-shrink-0" style="width: 28px;">
                    <input
                        type="checkbox"
                        class="form-check-input category-zone-parent-cb"
                        id="zone_cb_{{ $node['id'] }}"
                        value="{{ $node['id'] }}"
                        style="margin-left: 0 !important;">
                </div>

                <label class="form-check-label category-zone-node-label fw-medium text-muted mb-0"
                       for="zone_cb_{{ $node['id'] }}" style="flex: 1 1 auto; margin-left: 0.35rem;">{{ $node['name'] }}</label>
            </div>

            <div class="category-zone-tree-children d-none border-top border-light" style="background: rgba(0,0,0,.02);">
                <div class="py-2 px-2">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <div class="form-check py-2 mb-0 flex-shrink-0" style="width: 28px;">
                    <input
                        ...
                        name="zone_ids[]"
                        value="{{ $node['id'] }}"
                        class="form-check-input category-zone-leaf-cb"
                        id="zone_cb_{{ $node['id'] }}"
                        {{ in_array((string) $node['id'], $selectedStr, true) ? 'checked' : '' }}
                        style="margin-left: 0 !important;">
                </div>

                <label class="form-check-label category-zone-node-label text-muted mb-0"
                       for="zone_cb_{{ $node['id'] }}" style="flex: 1 1 auto; margin-left: 0.35rem;">{{ $node['name'] }}</label>
            </div>
        </div>
    @endif
