@php
    ...
    $transcriptLines = \Modules\BookingModule\Entities\BookingFollowup::parseTranscriptLines($transcript);
    $transcribeUrl = $hasRecording && $booking
        ? route('admin.booking.followup.transcribe', ['id' => $booking->id, 'followupId' => $followup->id])
        : '';
@endphp

<div class="voice-call-details-panel p-3"
     id="booking-followup-transcript-panel-{{ $followup->id }}"
            ...
            <div class="voice-call-left-stack d-flex flex-column gap-2 flex-grow-1">
                <div class="card voice-call-detail-box shadow-sm voice-call-recording-card">
                    ...
                    <div class="card-body voice-call-recording-box">
                        @if($hasRecording)
                            <audio controls preload="none" class="w-100 voice-call-audio-player" src="{{ $followup->recording_url }}">
                ...
                <div class="card voice-call-detail-box shadow-sm voice-call-summary-card">
                    <div class="card-header voice-call-detail-box__header">
                        ...
                        @if($summary !== '')
                            <button type="button" class="voice-call-copy-btn" title="{{ translate('Copy') }}" data-copy-b64="{{ base64_encode($summary) }}">
                    ...
                    <div class="card-body voice-call-summary-body">
                        ...
                        @else
                            ...
                            @if($hasRecording && $transcribeUrl !== '')
                                <button type="button"
                                        class="btn btn-sm btn--primary mt-2 js-transcribe-booking-followup-recording"
                                        data-followup-id="{{ $followup->id }}"
            ...
            <div class="card voice-call-detail-box shadow-sm flex-grow-1 voice-call-extracted-card">
                <div class="card-header voice-call-detail-box__header">
                    ...
                    @if($followup->hasTranscript() && $transcribeUrl !== '')
                        <button type="button"
                                class="btn btn-sm btn-outline-secondary js-transcribe-booking-followup-recording"
                                data-followup-id="{{ $followup->id }}"
                                data-url="{{ $transcribeUrl }}"
                                data-force="1"
                ...
                <div class="card-body voice-call-extracted-body">
                    <div class="voice-call-extracted-grid is-show-all">
                        ...
                        <div class="voice-call-extracted-item">
                            ...
                            <div class="voice-call-extracted-item__value">{{ $followup->followup_at?->format('d M Y, h:i A') ?? '—' }}</div>
                        </div>
                        <div class="voice-call-extracted-item">
    ...
    <div class="card voice-call-detail-box shadow-sm">
        <div class="card-header voice-call-detail-box__header">
            ...
            <div class="d-flex align-items-center gap-1">
                @if($transcript !== '')
                    <button type="button" class="voice-call-copy-btn voice-call-transcript-copy-btn" title="{{ translate('Copy') }}" data-copy-b64="{{ base64_encode($transcript) }}">
                        <span class="material-icons" aria-hidden="true">content_copy</span>
                    </button>
                @elseif($hasRecording && $transcribeUrl !== '')
                    <button type="button"
                            class="btn btn-sm btn--primary js-transcribe-booking-followup-recording"
                            data-followup-id="{{ $followup->id }}"
                            data-url="{{ $transcribeUrl }}"
                            data-has-transcript="0">
                        <span class="js-transcribe-btn-label">{{ translate('Transcribe_Recording') }}</span>
                ...
                <div class="voice-call-transcript followup-recording-transcript-wrap">
                    @foreach($transcriptLines as $line)
                        @php
                            ...
                            $lineClass = \Modules\BookingModule\Entities\BookingFollowup::transcriptLineClass($trimmedLine);
                        @endphp
                        @if($trimmedLine !== '')
