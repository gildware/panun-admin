                            ...
                            <form method="GET" action="{{ route('admin.booking.web-provider-requests.index') }}">
                                <div class="row g-3 align-items-center">
                                    ...
                                    <div class="col-md-4 d-flex justify-content-md-end gap-2">
                                        ...
                                        <a class="btn btn--secondary" href="{{ route('admin.booking.web-provider-requests.index') }}">
                                            {{ translate('Reset') }}
                                        </a>
                                    </div>
                                </div>
                                ...
                                <table class="table table-hover align-middle">
                                    ...
                                    @forelse($requests as $key => $providerRequest)
                                        <tr>
                                            ...
                                            <td>{{ $providerRequest->phone }}</td>
                                            <td>{{ $providerRequest->service_category ?: '—' }}</td>
                                            <td>{{ $providerRequest->area ?: '—' }}</td>
                                            <td>
                                                <span class="badge bg-info text-capitalize">
                                                    {{ str_replace('_', ' ', strtolower($providerRequest->status)) }}
                                                </span>
                                            </td>
                                            <td>
                                                @if($providerRequest->lead)
                                                    <a href="{{ route('admin.lead.show', $providerRequest->lead->id) }}?in_modal=1" class="link-primary">
                                                        #{{ $providerRequest->lead->id }} — {{ $providerRequest->lead->name ?: '—' }}
                                                    </a>
                                                    @if($providerRequest->lead->source)
                                                        <div class="text-muted small">{{ $providerRequest->lead->source->name }}</div>
                                                    @endif
                                                @else
                                                    —
                                                @endif
                                            </td>
                                            <td>{{ $providerRequest->created_at?->format('d M Y h:i a') }}</td>
                                            <td>
                                                <a href="{{ route('admin.booking.web-provider-requests.show', $providerRequest->id) }}" class="btn btn-sm btn--secondary">
                                                    {{ translate('View') }}
                                                </a>
                                            </td>
                                        </tr>
