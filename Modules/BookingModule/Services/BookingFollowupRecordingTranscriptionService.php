...
class BookingFollowupRecordingTranscriptionService
{
    /**
     * @return array{transcript: string, summary: string, transcribed_at: string, from_cache: bool}
     */
    public function transcribeAndSummarize(BookingFollowup $followup, bool $force = false): array
    {
        if (! $followup->hasRecording()) {
            throw new \InvalidArgumentException(translate('No_recording_available_for_this_follow_up'));
        }

        if (! $force && $followup->recording_transcript && $followup->recording_summary) {
            return [
                'transcript' => (string) $followup->recording_transcript,
                'summary' => (string) $followup->recording_summary,
                'transcribed_at' => $followup->transcribed_at?->toIso8601String() ?? '',
                'from_cache' => true,
            ];
        }
        ...
        [$audioBytes, $mimeType] = $this->loadRecording($followup);
        $result = $this->callGeminiAudio($apiKey, $audioBytes, $mimeType, $followup);

        $followup->recording_transcript = $result['transcript'];
        $followup->recording_summary = $result['summary'];
        $followup->transcribed_at = now();
        $followup->save();
     ...
     */
    private function loadRecording(BookingFollowup $followup): array
    {
        $disk = $followup->recording_disk ?: getDisk();
        $path = StoragePathPrefix::apply('booking-followups/'.$followup->recording_path);
        ...
        $mimeType = trim((string) ($followup->recording_mime ?: 'audio/wav'));
     ...
     */
    private function callGeminiAudio(string $apiKey, string $audioBytes, string $mimeType, BookingFollowup $followup): array
    {
        $system = <<<'PROMPT'
...
PROMPT;
        ...
        $context = trim(implode("\n", array_filter([
            $followup->remarks ? 'Agent remarks: '.$followup->remarks : null,
            $followup->contact_channel ? 'Channel: '.$followup->contact_channel : null,
            $followup->for ? 'Party: '.$followup->for : null,
        ])));
            ...
            try {
                ...
                if ($response->failed()) {
                    $lastError = $this->extractGeminiError($response->json(), $response->body())
                        ?: $lastError;
                    Log::warning('Booking follow-up transcription Gemini request failed', [
                ...
                Log::warning('Booking follow-up transcription exception', [
                    'followup_id' => $followup->id,
                    'model' => $model,
    ...
    private function parseJsonResponse(string $text): array
    {
        ...
        $transcript = BookingFollowup::formatTranscript(trim((string) ($decoded['transcript'] ?? '')));
    ...
    {
        $text = BookingFollowup::formatTranscript($text);
        if (mb_strlen($text) <= $max) {
            return $text;
        }
