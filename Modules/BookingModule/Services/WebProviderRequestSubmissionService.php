...
class WebProviderRequestSubmissionService
{
    ...
    /**
     * @param  array{name:string,phone:string,service:string,area:string,message:string,experience?:string|null}  $payload
     */
    public function submit(array $payload): WebProviderRequest
    {
        return DB::transaction(function () use ($payload) {
            $phone = $this->normalizePhone($payload['phone']);
            $lead = $this->ensureProviderLead($phone, $payload);
            $request = WebProviderRequest::create([
                ...
                'status' => WebProviderRequest::STATUS_PENDING_REVIEW,
                'lead_id' => $lead?->id,
            ]);
            ...
            admin_inbox_notify_web_provider_request_submitted($request);
     ...
     * @param  array{name:string,phone:string,service:string,area:string,message:string,experience?:string|null}  $payload
     */
    protected function ensureProviderLead(string $phone, array $payload): ?Lead
    {
        if ($phone === '') {
            return null;
        }
        ...
        $sourceId = Source::ensureWebsitePartnerApplicationSource()->id;
        ...
        if ($openLead) {
            ...
            if ($openLead->lead_type !== Lead::TYPE_PROVIDER) {
                $openLead->lead_type = Lead::TYPE_PROVIDER;
                $openLead->save();
        ...
        LeadTypeHistory::create([
            ...
            'data' => $this->providerHistoryData($payload),
            'created_by' => null,
        ]);
    ...
    protected function syncLeadIntakeData(Lead $lead, array $payload): void
    {
        ...
        $data = array_merge(
            ...
            $this->providerHistoryData($payload),
        );
    ...
    /**
     * @param  array{service:string,area:string,message:string,experience?:string|null}  $payload
     * @return array<string, mixed>
     */
    protected function providerHistoryData(array $payload): array
    {
        return array_filter([
            'provider_lead_status_id' => ProviderLeadStatus::defaultPendingStatusId(),
            ...
            'experience' => $payload['experience'] ?? null,
            'website_service_category' => trim($payload['service']),
    ...
    protected function buildRemarks(array $payload): string
    {
        $parts = array_filter([
            ...
            !empty($payload['experience']) ? 'Experience: ' . trim((string) $payload['experience']) : null,
    ...
    {
        do {
            $reference = 'WPR-' . now()->format('ymd') . '-' . strtoupper(Str::random(6));
        } while (WebProviderRequest::where('reference_id', $reference)->exists());
