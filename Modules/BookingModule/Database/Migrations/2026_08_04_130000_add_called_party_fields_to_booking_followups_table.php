...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('booking_followups', function (Blueprint $table) {
            if (! Schema::hasColumn('booking_followups', 'called_party_type')) {
                $table->string('called_party_type', 16)->nullable()->after('contact_channel');
            }
            if (! Schema::hasColumn('booking_followups', 'called_name')) {
                $table->string('called_name')->nullable()->after('called_party_type');
            }
            if (! Schema::hasColumn('booking_followups', 'called_number')) {
                $table->string('called_number', 32)->nullable()->after('called_name');
            }
            if (! Schema::hasColumn('booking_followups', 'called_provider_id')) {
    ...
    public function down(): void
    {
        Schema::table('booking_followups', function (Blueprint $table) {
            ...
            foreach ($columns as $column) {
                if (Schema::hasColumn('booking_followups', $column)) {
