        ...
        <div class="col-md-6 mb-4" style="cursor: pointer">
            <div class="card">
                <div class="card-body" style="height: 70px">
                    ...
                    @php($data->orderId = $payment_data->id)
                    @php($data->detail = $payment_data->id)
                    @php($data->amount = $payment_data->payment_amount)
                    @php($data->name = $payer->name??'')
                    @php($data->email = $payer->email ??'')
                    @php($data->phone = $payer->phone ??'')
                    @php($data->hashed_string = md5($secretkey . urldecode($data->detail) . urldecode($data->amount) . urldecode($data->orderId)))
