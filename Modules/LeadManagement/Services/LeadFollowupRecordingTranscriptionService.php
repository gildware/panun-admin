...
class LeadFollowupRecordingTranscriptionService
{
    /**
     * @return array{transcript: string, summary: string, transcribed_at: string, from_cache: bool}
     */
    public function transcribeAndSummarize(LeadFollowup $followup, bool $force = false): array
    {
     ...
     */
    private function loadRecording(LeadFollowup $followup): array
    {
        $disk = $followup->recording_disk ?: getDisk();
        $path = StoragePathPrefix::apply('lead-followups/'.$followup->recording_path);
     ...
     */
    private function callGeminiAudio(string $apiKey, string $audioBytes, string $mimeType, LeadFollowup $followup): array
    {
        $system = <<<'PROMPT'
            ...
            try {
                ...
                if ($response->failed()) {
                    $lastError = $this->extractGeminiError($response->json(), $response->body())
                        ?: $lastError;
                    Log::warning('Lead follow-up transcription Gemini request failed', [
                ...
                Log::warning('Lead follow-up transcription exception', [
                    'followup_id' => $followup->id,
                    'model' => $model,
