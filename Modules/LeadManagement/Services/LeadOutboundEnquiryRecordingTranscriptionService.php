...
class LeadOutboundEnquiryRecordingTranscriptionService
{
    /**
     * @return array{transcript: string, summary: string, transcribed_at: string, from_cache: bool}
     */
    public function transcribeAndSummarize(LeadOutboundEnquiry $enquiry, bool $force = false): array
    {
        if (! $enquiry->hasRecording()) {
            throw new \InvalidArgumentException(translate('No_outbound_enquiry_recording_for_transcription'));
        }

        if (! $force && $enquiry->recording_transcript && $enquiry->recording_summary) {
            return [
                'transcript' => (string) $enquiry->recording_transcript,
                'summary' => (string) $enquiry->recording_summary,
                'transcribed_at' => $enquiry->transcribed_at?->toIso8601String() ?? '',
                'from_cache' => true,
            ];
        }
        ...
        [$audioBytes, $mimeType] = $this->loadRecording($enquiry);
        $result = $this->callGeminiAudio($apiKey, $audioBytes, $mimeType, $enquiry);

        $enquiry->recording_transcript = $result['transcript'];
        $enquiry->recording_summary = $result['summary'];
        $enquiry->transcribed_at = now();
        $enquiry->save();
     ...
     */
    private function loadRecording(LeadOutboundEnquiry $enquiry): array
    {
        $disk = $enquiry->recording_disk ?: getDisk();
        $path = StoragePathPrefix::apply('lead-outbound-enquiries/'.$enquiry->recording_path);
        ...
        $mimeType = trim((string) ($enquiry->recording_mime ?: 'audio/wav'));
     ...
     */
    private function callGeminiAudio(string $apiKey, string $audioBytes, string $mimeType, LeadOutboundEnquiry $enquiry): array
    {
        $system = <<<'PROMPT'
You transcribe and summarize outbound enquiry phone calls for Panun Kaergar (home services marketplace in India).
...
PROMPT;
        ...
        $context = trim(implode("\n", array_filter([
            $enquiry->remarks ? 'Agent remarks: '.$enquiry->remarks : null,
            $enquiry->contacted_through ? 'Channel: '.$enquiry->contacted_through : null,
            $enquiry->customer_name ? 'Customer: '.$enquiry->customer_name : null,
        ])));
        ...
        $userParts = [
            ...
            [
                'text' => ($context !== '' ? $context."\n\n" : '')
                    .'Transcribe this support call recording and summarize it.',
            ],
        ];
            ...
            try {
                ...
                if ($response->failed()) {
                    $lastError = $this->extractGeminiError($response->json(), $response->body())
                        ?: $lastError;
                    Log::warning('Outbound enquiry transcription Gemini request failed', [
                ...
                Log::warning('Outbound enquiry transcription exception', [
                    'enquiry_id' => $enquiry->id,
                    'model' => $model,
