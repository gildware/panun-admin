@php
    $modalId = 'config-modal-' . $type;
    $hasColorField = in_array($type, ['provider_lead_status', 'customer_lead_status', 'customer_lead_tag']);
    $hasBaseTypeField = in_array($type, ['customer_lead_status', 'provider_lead_status'], true);
@endphp
            ...
            <table class="table table-sm align-middle mb-0">
                ...
                @forelse($items as $item)
                    <tr>
                        ...
                        @if($hasBaseTypeField)
                            <td class="text-capitalize">{{ $item->base_type ?? 'pending' }}</td>
                        @endif
                        @if($hasColorField)
                            <td>
                                <span class="d-inline-block rounded border" style="width: 24px; height: 24px; background-color: {{ $item->color ?? '#0d6efd' }};" title="{{ $item->color ?? '#0d6efd' }}"></span>
                                <span class="ms-1 small text-muted">{{ $item->color ?? '#0d6efd' }}</span>
                            </td>
                        @endif
                        ...
                        <td class="text-end">
                            <div class="d-inline-flex align-items-center gap-1">
                                <button type="button"
                                        ...
                                        data-config-active="{{ $item->is_active ? 1 : 0 }}"
                                        @if($hasColorField) data-config-color="{{ $item->color ?? '#0d6efd' }}" @endif
                ...
                @empty
                    <tr>
                        <td colspan="{{ ($hasBaseTypeField && $hasColorField) ? 7 : (($hasBaseTypeField || $hasColorField) ? 6 : 5) }}" class="text-center text-muted">{{ translate('No_data_found') }}</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        ...
        <div class="modal-content">
            ...
            <form method="POST" id="{{ $modalId }}-form">
                ...
                <div class="modal-body pt-0">
                    ...
                    @if($hasBaseTypeField)
                    <div class="mb-3">
                        <label class="form-label">{{ translate('Base_Type') }}</label>
                        <select name="base_type" class="form-select">
                            <option value="pending">{{ translate('Pending') }}</option>
                            <option value="cancel">{{ translate('Cancelled') }}</option>
                            <option value="completed">{{ translate('Completed') }}</option>
                        </select>
                    </div>
                    @endif
                    @if($hasColorField)
                    <div class="mb-3 provider-lead-status-color-wrap">
                        <label class="form-label">{{ translate('Color') }}</label>
                        <div class="d-flex align-items-center gap-2">
                            <input type="color" id="{{ $modalId }}-color-picker" class="form-control form-control-color p-1" value="#0d6efd" style="width: 48px; height: 38px;">
                            <input type="text" name="color" id="{{ $modalId }}-color" class="form-control" value="#0d6efd" maxlength="20" placeholder="#0d6efd" style="max-width: 120px;">
                        </div>
                        <small class="text-muted">{{ translate('Color_will_be_used_for_this_status_in_lead_details') }}</small>
                    </div>
                    @endif
...
@push('script')
    <script>
        (function () {
            ...
            if (modal) {
                const colorPicker = document.getElementById('{{ $modalId }}-color-picker');
                const colorInput = document.getElementById('{{ $modalId }}-color');
                ...
                modal.addEventListener('show.bs.modal', function (event) {
                    ...
                    const color = button.getAttribute('data-config-color') || '#0d6efd';
                    ...
                    if (action === 'edit' && id) {
                        form.action = '{{ route('admin.lead.configuration.update', '__id__') }}'.replace('__id__', id);
                        methodInput.value = 'PUT';
                        titleInput.value = title;
                        descriptionInput.value = description || '';
                        activeInput.checked = isActive;
                    } else {
                        form.action = '{{ route('admin.lead.configuration.store') }}';
            ...
            if (deleteModal) {
                deleteModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    form.action = '{{ route('admin.lead.configuration.destroy', '__id__') }}'.replace('__id__', id);
                    if (titleTarget) {
