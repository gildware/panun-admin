...
(function () {
    const analytics = {!! json_encode($customerLeadAnalytics) !!};
    const othersLabel = @json(translate('Others'));
    const leadsLabel = @json(translate('Leads'));
    const bookedLabel = @json(translate('Booked'));
    ...
    function renderAreaLine(el, categories, values, color) {
        ...
        new ApexCharts(el, {
            series: [{ name: bookedLabel, data: values }],
    ...
    renderDonut(
        document.querySelector('#customer-zone-chart'),
        zoneSlices.values,
        zoneSlices.labels,
        zoneSlices.colors
    );

    var dayLabels = analytics.lead_received_by_day_labels || [];
    var dayValues = analytics.lead_received_by_day || [];
    renderDonut(
    ...
    var bookedCat = topSlices(booked.category_wise || [], 5);
    var bookedZone = topSlices(booked.zone_wise || [], 5);
    var bookedSub = topSlices(booked.subcategory_wise || [], 5);

    renderDonut(document.querySelector('#customer-booked-category-chart'), bookedCat.values, bookedCat.labels, bookedCat.colors);
    renderDonut(document.querySelector('#customer-booked-zone-chart'), bookedZone.values, bookedZone.labels, bookedZone.colors);
    renderDonut(document.querySelector('#customer-booked-subcategory-chart'), bookedSub.values, bookedSub.labels, bookedSub.colors);
    ...
    renderDonut(
        document.querySelector('#customer-cancelled-category-chart'),
        cancelledCat.values,
        cancelledCat.labels,
        cancelledCat.colors
    );
    renderDonut(
        document.querySelector('#customer-cancelled-zone-chart'),
        cancelledZone.values,
        cancelledZone.labels,
        cancelledZone.colors
    );
    var cancelReasonSlices = topSlices(cancelled.reasons || [], 5);
    renderDonut(
        document.querySelector('#customer-cancel-reason-chart'),
        cancelReasonSlices.values,
        cancelReasonSlices.labels,
        ['#e74a3b', '#f6c23e', '#858796', '#4e73df', '#fd7e14', '#ced4da']
    );
})();
@endif
