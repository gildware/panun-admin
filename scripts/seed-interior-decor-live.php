...
/**
 * Seed Interior Decor catalog on live DB:
 * - Main category: Interior Decor Services
 * - Sub-categories: Home Decor Consultation / Commercial Decor Styling
 * - 4 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   bash scripts/assets/prepare-interior-decor-assets.sh
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-interior-decor-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=interior-decor,home-decor-consultation,commercial-decor-styling \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   INTERIOR_DECOR_DRY_RUN=1   validate assets only, no DB writes
 */
...
require_once base_path('scripts/lib/InteriorDecorContentBuilder.php');

$dryRun = filter_var(env('INTERIOR_DECOR_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('INTERIOR_DECOR_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/interior-decor-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('INTERIOR_DECOR_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'interior-decor'] as $slug) {
...
$liveConnection = 'live_interior_decor';
...
$ensureCategory = static function (array $spec, ?string $parentId, int $position) use (
    ...
    Category::on($liveConnection)->withoutGlobalScopes()->where('id', $category->id)->update(['slug' => $slug]);
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = InteriorDecorContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantTitle = in_array($variantSpec['title'], ['Book On Site Inspection', 'Book Site Visit', 'Book Site Inspection'], true)
            ? 'Book Site Visit'
            : $name.' — '.$variantSpec['title'];
        $variantDescription = InteriorDecorContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = InteriorDecorContentBuilder::variantNote($slug);
...
echo "\nInterior Decor catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
