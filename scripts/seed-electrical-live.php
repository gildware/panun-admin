...
/**
 * Seed / replace Electrician Services catalog on live DB.
 *
 * - Upserts main category (`electrical`) + 3 sub-categories
 * - Creates/refreshes services with overview, FAQs, images, priced variants
 * - Deactivates old electrical services/subs not in the new catalog (no hard delete)
 *
 * Prerequisites:
 *   python3 scripts/assets/electrical_icon_prompts.py
 *   python3 scripts/assets/electrical_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_electrical_ai_icons.py
 *   python3 scripts/assets/prepare_electrical_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py electrical electric-installation electric-repair electric-inspection
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-electrical-live.php
 *
 * Optional:
 *   ELECTRICAL_DRY_RUN=1
 *   ELECTRICAL_ONLY_SLUGS=electric-light-install,electric-fan-repair
 */
...
$dryRun = filter_var(env('ELECTRICAL_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('ELECTRICAL_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/electrical-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('ELECTRICAL_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'electrical'] as $slug) {
...
$liveConnection = 'live_electrical';
...
echo "\nElectrical catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
