...
/**
 * Seed Electric ACP Sign Board Install + Repair under live Electrician Services.
 *
 * Incremental only — does not rewrite other electrical services or deactivate anything.
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-acp-sign-board-live.php
 *
 * Optional:
 *   ACP_SIGN_BOARD_DRY_RUN=1
 */
...
require_once base_path('scripts/lib/ElectricalContentBuilder.php');

$dryRun = filter_var(env('ACP_SIGN_BOARD_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$targetSlugs = ['electric-acp-sign-board-install', 'electric-acp-sign-board-repair'];
$fullCatalog = require base_path('scripts/data/electrical-catalog.php');
...
if (count($catalogServices) !== 2) {
    throw new RuntimeException('ACP Sign Board catalog entries are missing from electrical-catalog.php.');
}
...
echo 'Asset validation passed for '.count($catalogServices)." ACP sign board services.\n";
...
$liveConnection = 'live_acp_sign_board';
...
if (! $mainCategory) {
    throw new RuntimeException('Electrician Services category (slug electrical) was not found on live.');
}

$subCategoryMap = [];
foreach (['electric-installation', 'electric-repair'] as $subSlug) {
    $sub = Category::on($liveConnection)->withoutGlobalScopes()
        ->where('slug', $subSlug)
        ->where('parent_id', $mainCategory->id)
        ->first();
    if (! $sub) {
        throw new RuntimeException("Missing electrician subcategory: {$subSlug}");
    }
...
foreach ($catalogServices as $serviceSpec) {
    ...
    $content = ElectricalContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = ElectricalContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = ElectricalContentBuilder::variantNote($slug);
...
echo "\nACP Sign Board catalog seeded under Electrician: ".count($catalogServices)." services.\n";
