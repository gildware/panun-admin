...
/**
 * Seed Book Kaergar catalog on live DB:
 * - Main category: Book Kaergar
 * - Sub-categories: Home Trades / Building & Site / Home Care / Beauty Artists
 * - Time-hire services with hour / half-day / full-day variants
 *
 * Prerequisites:
 *   python3 scripts/assets/book_kaergar_icon_prompts.py
 *   python3 scripts/assets/book_kaergar_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_book_kaergar_ai_icons.py
 *   python3 scripts/assets/prepare_book_kaergar_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py book-kaergar home-trades building-site home-care beauty-artists
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-book-kaergar-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=book-kaergar,home-trades,building-site,home-care,beauty-artists \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   BOOK_KAERGAR_DRY_RUN=1   validate assets only, no DB writes
 *   BOOK_KAERGAR_ONLY_SLUGS=book-a-carpenter,book-a-plumber
 */
...
require_once base_path('scripts/lib/BookKaergarContentBuilder.php');

$dryRun = filter_var(env('BOOK_KAERGAR_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('BOOK_KAERGAR_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/book-kaergar-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('BOOK_KAERGAR_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'book-kaergar'] as $slug) {
...
$liveConnection = 'live_book_kaergar';
...
$ensureCategory = static function (array $spec, ?string $parentId, int $position) use (
    ...
    if (! $category) {
        if ($dryRun) {
            echo "DRY RUN would create category: {$spec['name']} ({$slug})\n";
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = BookKaergarContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantTitle = $name.' — '.$variantSpec['title'];
        $variantDescription = BookKaergarContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = BookKaergarContentBuilder::variantNote($slug);
...
echo "\nBook Kaergar catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
