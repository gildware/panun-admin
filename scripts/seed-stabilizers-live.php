...
/**
 * Seed Stabilizers subcategory under live Home Appliances.
 *
 * Incremental only — does not rewrite other HA services or deactivate anything.
 *
 * Prerequisites:
 *   python3 scripts/assets/prepare_stabilizers_assets.py
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-stabilizers-live.php
 *
 * Optional:
 *   STABILIZERS_DRY_RUN=1
 */
...
require_once base_path('scripts/lib/HomeAppliancesContentBuilder.php');

$dryRun = filter_var(env('STABILIZERS_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$fullCatalog = require base_path('scripts/data/home-appliances-catalog.php');

$catalog = [
    'sub_categories' => array_values(array_filter(
        $fullCatalog['sub_categories'],
        static fn (array $sub): bool => ($sub['slug'] ?? '') === 'stabilizers'
    )),
    'services' => array_values(array_filter(
        $fullCatalog['services'],
        static fn (array $service): bool => ($service['sub_category_slug'] ?? '') === 'stabilizers'
    )),
];

if ($catalog['sub_categories'] === [] || $catalog['services'] === []) {
    throw new RuntimeException('Stabilizers catalog entries are missing from home-appliances-catalog.php.');
}
...
echo 'Asset validation passed for '.count($catalog['services'])." stabilizer services.\n";
...
$liveConnection = 'live_stabilizers';
...
if (! $mainCategory) {
    throw new RuntimeException('Home Appliances category (slug home-appliance) was not found on live.');
}

$ensureSubCategory = static function (array $spec, string $parentId) use (
    $liveConnection,
    $categoryIconDir,
    $uploadAsset,
...
$subCategoryMap = [];
foreach ($catalog['sub_categories'] as $subSpec) {
    $sub = $ensureSubCategory($subSpec, $mainCategory->id);
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = HomeAppliancesContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = HomeAppliancesContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = HomeAppliancesContentBuilder::variantNote($slug);
...
echo "\nStabilizers catalog seeded under Home Appliances: ".count($catalog['sub_categories']).' sub-category, '.count($catalog['services'])." services.\n";
