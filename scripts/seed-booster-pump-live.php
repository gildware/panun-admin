...
/**
 * Seed Booster Pump Install + Repair under live Plumbing Services.
 *
 * Incremental only — does not rewrite other plumbing services or deactivate anything.
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-booster-pump-live.php
 *
 * Optional:
 *   BOOSTER_PUMP_DRY_RUN=1
 */
...
require_once base_path('scripts/lib/PlumbingContentBuilder.php');

$dryRun = filter_var(env('BOOSTER_PUMP_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$boosterSlugs = ['booster-pump-install', 'booster-pump-repair'];
$fullCatalog = require base_path('scripts/data/plumbing-catalog.php');
...
if (count($catalogServices) !== 2) {
    throw new RuntimeException('Booster Pump catalog entries are missing from plumbing-catalog.php.');
}
...
echo 'Asset validation passed for '.count($catalogServices)." booster pump services.\n";
...
$liveConnection = 'live_booster_pump';
...
if (! $mainCategory) {
    throw new RuntimeException('Plumbing Services category (slug plumbing) was not found on live.');
}

$subCategoryMap = [];
foreach (['plumbing-installation', 'plumbing-repair'] as $subSlug) {
    $sub = Category::on($liveConnection)->withoutGlobalScopes()
        ->where('slug', $subSlug)
        ->where('parent_id', $mainCategory->id)
        ->first();
    if (! $sub) {
        throw new RuntimeException("Missing plumbing subcategory: {$subSlug}");
    }
...
foreach ($catalogServices as $serviceSpec) {
    ...
    $content = PlumbingContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = PlumbingContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = PlumbingContentBuilder::variantNote($slug);
...
echo "\nBooster Pump catalog seeded under Plumbing: ".count($catalogServices)." services.\n";
