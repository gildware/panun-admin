...
/**
 * Seed Pest Control catalog on live DB:
 * - Main category: Pest Control
 * - Sub-categories: Home / Office / Restaurant Pest Control
 * - 10 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   python3 scripts/assets/generate_pest_control_assets.py
 *     (photoreal 1024x1024 / 1536x1024 service images + ref-style variant icons + light/dark category pairs)
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-pest-control-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=pest-control,home-pest-control,office-pest-control,restaurant-pest-control \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   PEST_CONTROL_DRY_RUN=1   validate assets only, no DB writes
 */
...
$dryRun = filter_var(env('PEST_CONTROL_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('PEST_CONTROL_ONLY_SLUGS', ''))));
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('PEST_CONTROL_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'pest-control'] as $slug) {
...
$liveConnection = 'live_pest_control';
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = PestControlContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = PestControlContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = PestControlContentBuilder::variantNote($slug);
...
echo "\nPest Control catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
