#!/usr/bin/env python3
"""Post-process AI-generated pet grooming category + variant icons."""
...
CAT_OUT = ROOT / "assets" / "category-icons"
VARIANT_OUT = ROOT / "assets" / "variant-icons"
PROMPTS = ROOT / "assets" / "data" / "pet-grooming-icon-prompts.json"
...
def main() -> None:
    if not PROMPTS.is_file():
        subprocess.run([sys.executable, str(ROOT / "assets" / "pet_grooming_icon_prompts.py")], check=True)
    ...
    if missing:
        print(f"Missing {len(missing)} icon sources:")
        for name in missing:
            print(f"  - {name}")
        sys.exit(2)

    print("All pet grooming icons prepared.")
