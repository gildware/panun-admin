...
/**
 * Seed / replace Men's Salon catalog on live DB.
 *
 * - Upserts main category (`mens-salon`) + 3 sub-categories
 * - Creates/refreshes services with overview, FAQs, images, priced variants
 * - Deactivates old men's salon services/subs not in the new catalog (no hard delete)
 *
 * Prerequisites:
 *   python3 scripts/assets/mens_salon_icon_prompts.py
 *   python3 scripts/assets/mens_salon_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_mens_salon_ai_icons.py
 *   python3 scripts/assets/prepare_mens_salon_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py mens-salon mens-hair-services mens-beard-shaving mens-skin-grooming-care
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-mens-salon-live.php
 *
 * Optional:
 *   MENS_SALON_DRY_RUN=1
 *   MENS_SALON_ONLY_SLUGS=mens-hair-cut,mens-beard-trimming
 */
...
require_once base_path('scripts/lib/MensSalonContentBuilder.php');

$dryRun = filter_var(env('MENS_SALON_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('MENS_SALON_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/mens-salon-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('MENS_SALON_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'mens-salon'] as $slug) {
...
$liveConnection = 'live_mens_salon';
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = MensSalonContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = MensSalonContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = MensSalonContentBuilder::variantNote($slug);
...
echo "\nMen's Salon catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
