...
/**
 * Seed Wooden Flooring Install + Repair under live Carpentry Services.
 *
 * Incremental only — does not rewrite other carpentry services or deactivate anything.
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-wooden-flooring-live.php
 *
 * Optional:
 *   WOODEN_FLOORING_DRY_RUN=1
 */
...
$dryRun = filter_var(env('WOODEN_FLOORING_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$flooringSlugs = ['wooden-flooring-install', 'wooden-flooring-repair'];
$fullCatalog = require base_path('scripts/data/carpentry-catalog.php');
$catalogServices = array_values(array_filter(
    $fullCatalog['services'],
    static fn (array $service): bool => in_array($service['slug'] ?? '', $flooringSlugs, true)
));

if (count($catalogServices) !== 2) {
    throw new RuntimeException('Wooden Flooring catalog entries are missing from carpentry-catalog.php.');
}
...
foreach ($catalogServices as $serviceSpec) {
...
echo 'Asset validation passed for '.count($catalogServices)." wooden flooring services.\n";
...
$liveConnection = 'live_wooden_flooring';
...
$mainCategory = Category::on($liveConnection)->withoutGlobalScopes()
    ->where('slug', 'carpentary')
    ->whereNull('parent_id')
    ->first();

if (! $mainCategory) {
    throw new RuntimeException('Carpentry Services category (slug carpentary) was not found on live.');
}

$subCategoryMap = [];
foreach (['carpentry-installation', 'carpentry-repairs'] as $subSlug) {
    $sub = Category::on($liveConnection)->withoutGlobalScopes()
        ->where('slug', $subSlug)
        ->where('parent_id', $mainCategory->id)
        ->first();
    if (! $sub) {
        throw new RuntimeException("Missing carpentry subcategory: {$subSlug}");
    }
    $subCategoryMap[$subSlug] = $sub->id;
}
...
foreach ($catalogServices as $serviceSpec) {
    ...
    foreach (['name' => $name, 'short_description' => $content['short_description'], 'description' => $content['description']] as $field => $value) {
...
$placeAfter = [
    'wooden-flooring-install' => 'table-installation',
    'wooden-flooring-repair' => 'door-repair',
];

foreach ($placeAfter as $slug => $afterSlug) {
    $service = Service::on($liveConnection)->withoutGlobalScopes()->where('slug', $slug)->first();
    $anchor = Service::on($liveConnection)->withoutGlobalScopes()->where('slug', $afterSlug)->first();
    if (! $service || ! $anchor || $service->sub_category_id !== $anchor->sub_category_id) {
        continue;
    }

    $newSort = (int) $anchor->sort_order + 1;
    if ((int) $service->sort_order === $newSort) {
        continue;
    }

    Service::on($liveConnection)->withoutGlobalScopes()
        ->where('sub_category_id', $service->sub_category_id)
        ->where('id', '!=', $service->id)
        ->where('sort_order', '>=', $newSort)
        ->increment('sort_order');

    Service::on($liveConnection)->withoutGlobalScopes()->where('id', $service->id)->update(['sort_order' => $newSort]);
    echo "Sorted {$slug} after {$afterSlug} (sort {$newSort})\n";
}
...
echo "\nWooden Flooring catalog seeded under Carpentry: ".count($catalogServices)." services.\n";
