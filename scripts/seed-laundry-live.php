...
/**
 * Seed / replace Dry Cleaning & Laundry catalog on live DB.
 *
 * - Upserts main Dry Cleaning & Laundry category + 2 sub-categories
 * - Creates/refreshes 6 services with overview, FAQs, images, variants @ ₹50
 * - Deactivates old laundry services not in the new catalog (no hard delete)
 * - Deactivates obsolete sub-categories (none currently)
 *
 * Prerequisites:
 *   python3 scripts/assets/laundry_icon_prompts.py
 *   python3 scripts/assets/laundry_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_laundry_ai_icons.py
 *   python3 scripts/assets/prepare_laundry_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py laundry wash-laundry dry-clean
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-laundry-live.php
 *
 * Optional:
 *   LAUNDRY_DRY_RUN=1
 *   LAUNDRY_ONLY_SLUGS=clothing-laundry,garment-dry-cleaning
 */
...
require_once base_path('scripts/lib/LaundryContentBuilder.php');

$dryRun = filter_var(env('LAUNDRY_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('LAUNDRY_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/laundry-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('LAUNDRY_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'cleaning'] as $slug) {
...
$ensureCategory = static function (array $spec, ?string $parentId, int $position) use (
    ...
    if (! $category && $parentId) {
        // Name-based fallback: Category model rewrites slug from name on update
        // (e.g. Laundry under laundry → laundry-1), so also match by parent + name.
        $category = Category::on($liveConnection)->withoutGlobalScopes()
    ...
    // Force catalog slug — model updating hook regenerates slug from name.
    Category::on($liveConnection)->withoutGlobalScopes()->where('id', $category->id)->update([
        'slug' => $slug,
        'name' => $spec['name'],
        'parent_id' => $parentId,
        'position' => $position,
    ...
    // Prefer dark theme pair when available (app dark mode).
    $darkIconPath = base_path('scripts/category-icons/dark/'.$slug.'.png');
    if (is_file($darkIconPath)) {
    ...
    return Category::on($liveConnection)->withoutGlobalScopes()->find($category->id);
};
...
// Deactivate obsolete sub-categories (keep rows for history / subscriptions)
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = LaundryContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = LaundryContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = LaundryContentBuilder::variantNote($slug);
...
// Deactivate old laundry services not in the new catalog (preserve booking history)
...
echo "\nLaundry catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
