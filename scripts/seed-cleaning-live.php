...
/**
 * Seed / replace Cleaning Services catalog on live DB.
 *
 * - Upserts main Cleaning Services category + 4 sub-categories
 * - Creates/refreshes 18 services with overview, FAQs, images, variants @ ₹50
 * - Deactivates old cleaning services not in the new catalog (no hard delete)
 * - Deactivates obsolete sub-categories (home-cleaning, office-commercial-cleaning)
 *
 * Prerequisites:
 *   python3 scripts/assets/cleaning_icon_prompts.py
 *   python3 scripts/assets/cleaning_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_cleaning_ai_icons.py
 *   python3 scripts/assets/prepare_cleaning_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py cleaning home-commercial-cleaning furniture-fabric-cleaning appliance-utility-cleaning post-construction-cleaning
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-cleaning-live.php
 *
 * Optional:
 *   CLEANING_DRY_RUN=1
 *   CLEANING_ONLY_SLUGS=bathroom-cleaning,sofa-cleaning
 */
...
require_once base_path('scripts/lib/CleaningContentBuilder.php');

$dryRun = filter_var(env('CLEANING_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('CLEANING_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/cleaning-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('CLEANING_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = CleaningContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = CleaningContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = CleaningContentBuilder::variantNote($slug);
...
// Deactivate old cleaning services not in the new catalog (preserve booking history)
...
echo "\nCleaning catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
