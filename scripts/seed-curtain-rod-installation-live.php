...
/**
 * Seed Curtain Rod Installation under live Carpentry Services.
 *
 * Incremental only — does not rewrite other carpentry services or deactivate anything.
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-curtain-rod-installation-live.php
 *
 * Optional:
 *   CURTAIN_ROD_DRY_RUN=1
 */
...
$dryRun = filter_var(env('CURTAIN_ROD_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$targetSlugs = ['curtain-rod-installation'];
...
$catalogServices = array_values(array_filter(
    ...
    static fn (array $service): bool => in_array($service['slug'] ?? '', $targetSlugs, true)
));

if (count($catalogServices) !== 1) {
    throw new RuntimeException('Curtain Rod Installation catalog entry is missing from carpentry-catalog.php.');
}
...
echo 'Asset validation passed for '.count($catalogServices)." curtain rod service.\n";
...
$liveConnection = 'live_curtain_rod';
...
$subCategoryMap = [];
foreach (['carpentry-installation'] as $subSlug) {
...
$placeAfter = [
    'curtain-rod-installation' => 'wooden-flooring-install',
];
...
echo "\nCurtain Rod Installation seeded under Carpentry: ".count($catalogServices)." service.\n";
