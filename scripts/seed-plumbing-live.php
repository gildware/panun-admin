...
/**
 * Seed / replace Plumbing Services catalog on live DB.
 *
 * - Upserts main category (`plumbing`) + 3 sub-categories
 * - Creates/refreshes services with overview, FAQs, images, priced variants
 * - Deactivates old plumbing services/subs not in the new catalog (no hard delete)
 *
 * Prerequisites:
 *   python3 scripts/assets/plumbing_icon_prompts.py
 *   python3 scripts/assets/plumbing_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_plumbing_ai_icons.py
 *   python3 scripts/assets/prepare_plumbing_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py plumbing plumbing-installation plumbing-repair plumbing-inspection
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-plumbing-live.php
 *
 * Optional:
 *   PLUMBING_DRY_RUN=1
 *   PLUMBING_ONLY_SLUGS=plumbing-tap-install,plumbing-drain-repair
 */
...
$dryRun = filter_var(env('PLUMBING_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('PLUMBING_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'plumbing'] as $slug) {
...
echo "\nPlumbing catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
