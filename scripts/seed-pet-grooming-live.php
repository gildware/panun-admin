...
/**
 * Seed Pet Grooming catalog on live DB:
 * - Main category: Pet Grooming
 * - Sub-categories: Dog Grooming / Cat Grooming
 * - 23 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   bash scripts/assets/prepare-pet-grooming-assets.sh
 *
...
require_once base_path('scripts/lib/PetGroomingContentBuilder.php');

$dryRun = filter_var(env('PET_GROOMING_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('PET_GROOMING_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/pet-grooming-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('PET_GROOMING_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'pet-grooming'] as $slug) {
...
$liveConnection = 'live_pet_grooming';
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = PetGroomingContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = PetGroomingContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = PetGroomingContentBuilder::variantNote($slug);
