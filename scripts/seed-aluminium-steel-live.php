...
/**
 * Seed Aluminium & Steel Works catalog on live DB:
 * - Main category: Aluminium & Steel Works
 * - Sub-categories: Installation / Repairs & Maintenance / Fabrication
 * - 29 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   python3 scripts/assets/generate_aluminium_steel_assets.py
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-aluminium-steel-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=aluminium-steel-works,metal-works-installation,metal-works-repairs,metal-works-fabrication \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   ALUMINIUM_STEEL_DRY_RUN=1   validate assets only, no DB writes
 */
...
$dryRun = filter_var(env('ALUMINIUM_STEEL_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('ALUMINIUM_STEEL_ONLY_SLUGS', ''))));
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('ALUMINIUM_STEEL_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'aluminium-steel-works'] as $slug) {
...
$liveConnection = 'live_aluminium_steel';
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = AluminiumSteelContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = AluminiumSteelContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = AluminiumSteelContentBuilder::variantNote($slug);
...
echo "\nAluminium & Steel Works catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
