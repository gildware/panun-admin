...
/**
 * Seed Generators subcategory under live Home Appliances.
 *
 * Incremental only — does not rewrite other HA services or deactivate anything.
 *
 * Prerequisites:
 *   python3 scripts/assets/generate_generators_assets.py
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-generators-live.php
 *
 * Optional:
 *   GENERATORS_DRY_RUN=1
 */
...
$dryRun = filter_var(env('GENERATORS_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
...
if ($catalog['sub_categories'] === [] || $catalog['services'] === []) {
    throw new RuntimeException('Generators catalog entries are missing from home-appliances-catalog.php.');
}
...
echo 'Asset validation passed for '.count($catalog['services'])." generator services.\n";
...
$liveConnection = 'live_generators';
...
echo "\nGenerators catalog seeded under Home Appliances: ".count($catalog['sub_categories']).' sub-category, '.count($catalog['services'])." services.\n";
