...
/**
 * Seed / replace Masonry Services catalog on live DB.
 *
 * - Upserts main category (`masonry`) + 3 sub-categories
 * - Creates/refreshes services with overview, FAQs, images, priced variants
 * - Deactivates old masonry services/subs not in the new catalog (no hard delete)
 *
 * Prerequisites:
 *   python3 scripts/assets/masonry_icon_prompts.py
 *   python3 scripts/assets/masonry_photo_prompts.py
 *   # Generate AI icons + photos into Cursor assets/, then:
 *   python3 scripts/prepare_masonry_ai_icons.py
 *   python3 scripts/assets/prepare_masonry_assets.py
 *   python3 scripts/assets/category-icons/make_theme_pairs.py masonry masonry-installation masonry-repair masonry-inspection
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-masonry-live.php
 *
 * Optional:
 *   MASONRY_DRY_RUN=1
 *   MASONRY_ONLY_SLUGS=masonry-brick-install,masonry-crack-repair
 */
...
require_once base_path('scripts/lib/MasonryContentBuilder.php');

$dryRun = filter_var(env('MASONRY_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('MASONRY_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/masonry-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('MASONRY_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'masonry'] as $slug) {
...
// Prefer live DB for BusinessSettings / R2 credentials when local MySQL is unavailable.
$originalDefaultConnection = (string) config('database.default');
config(['database.default' => $liveConnection]);
DB::purge($originalDefaultConnection);
DB::connection($liveConnection)->getPdo();

CloudStorageConfigurator::apply();

$prefixSetting = BusinessSettings::on($liveConnection)
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = MasonryContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = MasonryContentBuilder::variantDescription($name, $variantTitle);
        $variantNote = MasonryContentBuilder::variantNote($slug);
...
DB::purge($liveConnection);

echo "\nMasonry catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
