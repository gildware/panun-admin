...
/**
 * Seed Vehicle Services catalog on live DB:
 * - Main category: Vehicle Services
 * - Sub-categories: Car Wash & Detailing / Car Repair & Maintenance / Bike & Scooter Service
 * - 13 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   Generate assets via prompts into scripts/assets/ (category-icons, service-images, variant-icons)
 *   See scripts/assets/data/vehicle-services-icon-prompts.json and vehicle-services-photo-prompts.json
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-vehicle-services-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=vehicle-services,car-wash-detailing,car-repair-maintenance,bike-scooter-service \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   VEHICLE_SERVICES_DRY_RUN=1   validate assets only, no DB writes
 */
...
require_once base_path('scripts/lib/VehicleServicesContentBuilder.php');

$dryRun = filter_var(env('VEHICLE_SERVICES_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('VEHICLE_SERVICES_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/vehicle-services-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('VEHICLE_SERVICES_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'vehicle-services'] as $slug) {
...
$liveConnection = 'live_vehicle_services';
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = VehicleServicesContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantTitle = $variantSpec['title'] === 'Book On Site Inspection'
            ? 'Book On Site Inspection'
            : $name.' — '.$variantSpec['title'];
        $variantDescription = VehicleServicesContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = VehicleServicesContentBuilder::variantNote($slug);
...
echo "\nVehicle Services catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
