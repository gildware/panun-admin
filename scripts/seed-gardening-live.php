...
/**
 * Seed Gardening catalog on live DB:
 * - Main category: Gardening Services
 * - Sub-categories: Lawn & Grass Care / Planting & Soil Care / Pruning & Trimming / Garden Cleanup & Maintenance
 * - 15 services with content, images, FAQs, and zone variants
 *
 * Prerequisites:
 *   Generate assets via prompts into scripts/assets/ (category-icons, service-images, variant-icons)
 *
 * LIVE_DB_PASSWORD='...' php artisan tinker scripts/seed-gardening-live.php
 *
 * Category light/dark icons (project standard):
 *   SLUGS=gardening,lawn-grass-care,planting-soil-care,pruning-trimming,garden-cleanup-maintenance \
 *     LIVE_DB_PASSWORD='...' php artisan tinker scripts/upload-category-icons-live-slugs.php
 *
 * Optional:
 *   GARDENING_DRY_RUN=1   validate assets only, no DB writes
 */
...
require_once base_path('scripts/lib/GardeningContentBuilder.php');

$dryRun = filter_var(env('GARDENING_DRY_RUN', false), FILTER_VALIDATE_BOOLEAN);
$onlySlugs = array_filter(array_map('trim', explode(',', (string) env('GARDENING_ONLY_SLUGS', ''))));
$catalog = require base_path('scripts/data/gardening-catalog.php');
...
if ($onlySlugs !== []) {
    ...
    if ($catalog['services'] === []) {
        throw new RuntimeException('GARDENING_ONLY_SLUGS did not match any catalog services.');
    }
}
...
foreach ([$catalog['category']['slug'] ?? 'gardening'] as $slug) {
...
foreach ($catalog['services'] as $serviceSpec) {
    ...
    $content = GardeningContentBuilder::build($serviceSpec);
    $assetsDir = $serviceImageDir.'/'.$slug;
    ...
    foreach ($serviceSpec['variants'] as $variantSpec) {
        ...
        $variantDescription = GardeningContentBuilder::variantDescription($name, $variantSpec['title']);
        $variantNote = GardeningContentBuilder::variantNote($slug);
...
echo "\nGardening catalog seeded: 1 category, ".count($catalog['sub_categories']).' sub-categories, '.count($catalog['services'])." services.\n";
