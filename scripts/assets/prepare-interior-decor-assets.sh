#!/bin/bash
# Prepare interior decor assets from Cursor assets folder (prompt-generated images).
set -euo pipefail
...
CATALOG="$ROOT/scripts/data/interior-decor-catalog.php"

mkdir -p "$CAT_ICONS" "$SERVICE_IMG" "$VARIANT_ICONS" "$LIGHT_DIR" "$DARK_DIR"

echo "==> Category icons"
for slug in interior-decor home-decor-consultation commercial-decor-styling; do
  src="$SRC/${slug}.png"
  if [[ ! -f "$src" ]]; then
...
python3 "$THEME_SCRIPT" interior-decor home-decor-consultation commercial-decor-styling

echo "All interior decor assets ready."
